...
categorical_vars = names(df)[!names(df) %in% union(numerical_vars, target)]
```
...
#   idx = get_outliers_idx(df[[col]])
#   df[idx, col] = median(df[[col]], na.rm = TRUE)
# }
...
*Pairplots*
```{r}
pairs(df[numerical_vars], col = "steelblue")
```

*Correlation*
```{r}
round(cor(df[numerical_vars]), 2)
```
...
```{r}
df$room_shared = ifelse(df$room_shared == "True", 1, 0)
df$room_private = ifelse(df$room_private == "True", 1, 0) 
...
```{r}
df = subset(df, select = -c(attr_index, rest_index, room_shared, room_private))
```
...
bin_width = (max(df$realSum) - min(df$realSum)) / num_bins

breakpoints = seq(min(df$realSum), max(df$realSum), by = bin_width)

hist(df$realSum[df$realSum < max(df$realSum)],
...
## Finding the best additive model
...
```{r}
idx = sample(1:nrow(df), floor(0.8 * nrow(df)))
...
```{r}
res_df = data.frame(predictors = character(),
                        rmse = numeric(),
                        stringsAsFactors = FALSE)

predictors = names(df)[!(names(df) %in% target)]

for (size in 1:length(numerical_vars)) {
  combinations = combn(predictors, size, simplify = FALSE)
  for (i in 1:length(combinations)) {
    model = lm(as.formula(paste("realSum ~ ", paste(combinations[[i]], collapse = "+"))), data = train_data)
    
    pred = predict(model, newdata = X_test)

    rmse = calc_rmse(y_test, pred)
    
    res_df = rbind(res_df, data.frame(predictors = paste(combinations[[i]], collapse = ","), rmse = rmse)) 
  }
}

res_df[which.min(res_df$rmse),]
```
